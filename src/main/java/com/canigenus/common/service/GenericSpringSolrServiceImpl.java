 ...
 * 
 *//*
public abstract class GenericSpringSolrServiceImpl implements GenericService {
	...
	public <T> T getPartialModel(Class<T> clazz, Object id,
	  ...
	  Query query= new Query();
		...
		CriteriaBuilder criteriaBuilder = getSolrTemplate()
				.getCriteriaBuilder();
		...
		return getSolrTemplate().createQuery(cq).getSingleResult();
	}
	...
	public <T> List<T> getPartialModelList(Class<T> clazz,
		...
		return getSolrTemplate().createQuery(criteriaQuery).getResultList();
	}

	@Override
	public <T> Long getModelCount(Class<T> clazz,
			CriteriaPopulator criteriaPopulator) {
		CriteriaBuilder cb = getSolrTemplate().getCriteriaBuilder();
	...
	public <T, U extends Number> U getSumByColumn(Class<T> clazz,
		...
		TypedQuery<U> typedQuery = getSolrTemplate().createQuery(cq);
		if (start > 0) {
	...
	public <T> Long getMaxByColumn(Class<T> clazz, String column) {
		...
		TypedQuery<Long> typedQuery = getSolrTemplate().createQuery(cq);
	...
	public <T> List<T> getPartialModelListWithJoin(Class<T> clazz,
		...
		TypedQuery<T> typedQuery = getSolrTemplate().createQuery(cq);
		if (maxResult > 0) {
	...
	public <T> List<T> getModelList(Class<T> clazz, String filterFieldName,
		...
		cq.orderBy(cb.desc(r.get("id")));
		return getSolrTemplate().createQuery(cq).getResultList();
	}
	...
	public <T> T merge(T model) {
		return getSolrTemplate().merge(model);
	}

	@Override
	public <T> void persist(T model) {
		getSolrTemplate().persist(model);
	}

	@Override
	public <T> void refresh(T model) {
		if (!getSolrTemplate().contains(model)) {
			getSolrTemplate().merge(model);
		}
		getSolrTemplate().refresh(model);
	}

	@Override
	public <T> void persist(T model, boolean clear) {
		if (clear)
			getSolrTemplate().clear();
		getSolrTemplate().merge(model);
	}

	@Override
	public <T> void delete(T model) {
		getSolrTemplate().remove(model);
	}

	@Override
	public <T> T getModel(Object id, Class<T> classType) {
		T t = getSolrTemplate().find(classType, id);
		return t;
	}
	...
	public <T> T getModelWithDepth(Object id, Class<T> type,
		...
		return getSolrTemplate().createQuery(criteriaQuery).getSingleResult();
	}

	@Override
	public <T> void deleteDetached(T model) {
		//getSolrTemplate().delete(model);
		SolrDataQuery solrDataQuery;
